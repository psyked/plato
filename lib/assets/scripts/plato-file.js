...
$(function(){
  ...
  var popovers = cm.operation(function(){
    ...
    __report.complexity.functions.forEach(function(fn,i){
      ...
      if(fn.complexity.cyclomatic > maxComplexity){
        maxComplexity = fn.complexity.cyclomatic;
      }
      bySloc.push({
        label : fn.name,
      ...
      if(fn.complexity.sloc.physical > maxSloc){
        maxSloc = fn.complexity.sloc.physical;
      }
  ...
  function GreenYellowRed(val) {
    var r,g,b;
        r = ((510 - (val/100) * 255 * 2) < 255) ? (510 - (val/100) * 255 * 2) : 255;
        g = (((val/100) * 255 * 2) < 255) ? ((val/100) * 255 * 2) : 255;
        b = 0;
        return "rgb("+r+","+g+","+b+")"
  }
  ...
  var colours = [];
  for(var i=0,l=byComplexity.length;i<l;i++){
    colours.push(GreenYellowRed(100 - ((100 / maxComplexity) * byComplexity[i].value)));
  }
  var slocColours = [];
  for(var i=0,l=bySloc.length;i<l;i++){
    slocColours.push(GreenYellowRed(100 - ((100 / maxSloc) * bySloc[i].value)));
  }
  // yield to the browser
  setTimeout(function(){
    drawCharts([
      { element: 'fn-by-complexity', data: byComplexity, colors: colours },
      { element: 'fn-by-sloc', data: bySloc, colors: slocColours }
    ]);
  },0);
